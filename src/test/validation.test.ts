...
describe('Validation', () => {
  ...
  describe('Complex validation', () => {
    test('should validate arrays with custom functions', () => {
      ...
      const maxLengthSchema = s.array(s.number()).validate(arr => arr.length <= 3);
    ...
    test('should validate objects with custom functions', () => {
      ...
      assert.strictEqual(userSchema.runValidation({ name: 'John', age: 25 }), true);
      assert.strictEqual(userSchema.runValidation({ name: '', age: 25 }), false);
      assert.strictEqual(userSchema.runValidation({ name: 'John', age: -1 }), false);
    });
  });
  ...
  describe('Real-world validation examples', () => {
    test('should validate email addresses', () => {
      ...
      assert.strictEqual(emailSchema.runValidation('test.email@domain.co.uk'), true);
      assert.strictEqual(emailSchema.runValidation('invalid-email'), false);
    ...
    test('should validate usernames', () => {
      ...
      assert.strictEqual(usernameSchema.runValidation('this_is_a_very_long_username_that_exceeds_limit'), false); // too long
      assert.strictEqual(usernameSchema.runValidation('invalid-username'), false); // contains hyphen
    });

    test('should validate age ranges', () => {
      const adultAgeSchema = s.number().validate(age => age >= 18 && age <= 120);
