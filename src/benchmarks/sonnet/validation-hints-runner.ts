...
function validateUserProfile(result: any): {
  isValid: boolean;
  score: number;
  errors: string[];
} {
...
${userProfileSchemaWithValidation.stringify()}
...
export async function runComparison(
  ...
  const totalStructLMTokens =
    structlmWithValidationResult.totalInputTokens +
    structlmWithValidationResult.totalOutputTokens;

  const tokenSavings = {
    inputTokens:
      jsonSchemaResult.totalInputTokens -
      structlmWithValidationResult.totalInputTokens,
    outputTokens:
      jsonSchemaResult.totalOutputTokens -
      structlmWithValidationResult.totalOutputTokens,
  ...
  const accuracyDifference =
    structlmWithValidationResult.accuracyScore - jsonSchemaResult.accuracyScore;
  const performanceDifference =
    jsonSchemaResult.averageResponseTime -
    structlmWithValidationResult.averageResponseTime;
...
export function printResults(comparison: ComparisonResult): void {
  ...
  console.log(
    `  Successful: ${comparison.structlmWithValidation.successfulIterations}/${comparison.structlmWithValidation.totalIterations}`
  );
  console.log(
    `  Average Input Tokens: ${comparison.structlmWithValidation.averageInputTokens}`
  );
  console.log(
    `  Average Output Tokens: ${comparison.structlmWithValidation.averageOutputTokens}`
  );
  console.log(
    `  Total Tokens: ${comparison.structlmWithValidation.totalInputTokens + comparison.structlmWithValidation.totalOutputTokens}`
  );
  console.log(
    `  Accuracy Score: ${comparison.structlmWithValidation.accuracyScore}%`
  );
  console.log(
    `  Average Response Time: ${comparison.structlmWithValidation.averageResponseTime}ms`
  );
  ...
  if (comparison.structlmWithValidation.errors.length > 0) {
    ...
    comparison.structlmWithValidation.errors.forEach(error =>
      console.log(`  - ${error}`)
    );
  }
  ...
  console.log(
    `  • StructLM with validation hints saves ${comparison.tokenSavings.percentageSaved}% tokens vs JSON Schema`
  );
  console.log(
    `  • Validation hints provide clear constraints while maintaining efficiency`
  );
  console.log(
    `  • Accuracy difference: ${comparison.accuracyDifference > 0 ? '+' : ''}${comparison.accuracyDifference}% compared to JSON Schema`
  );
  console.log('='.repeat(80));
}
